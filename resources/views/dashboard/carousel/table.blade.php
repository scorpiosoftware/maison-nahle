<x-app-layout>
    ...
    <div class="py-6 group">
        <h1
            ...
            {{ session('lang') == 'en' ? 'Carousel View' : 'واجة السلايدر' }}
        </h1>
    </div>
    <div class="relative m-4 md:flex justify-between items-center">
        <a href="{{ route('carousel.create') }}"> <button type="button"
                class="focus:outline-none shadow-lg text-white bg-blue-600 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 d:bg-green-600 d:hover:bg-green-700 d:focus:ring-green-800">
                ...
                @foreach ($records as $record)
                    <tr class="bg-white border-b d:bg-gray-800 d:border-gray-700">
                        ...
                        <td class="px-6 py-3 text-right flex justify-start items-center pt-[30px] space-x-4">
                            ...
                            <form action="{{ route('carousel.destroy', $record->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button
