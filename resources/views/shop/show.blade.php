    ...
    <div class="mx-auto max-w-7xl p-4 mt-4">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
            ...
            <div class="relative" x-data="{
                ...
                <div class="relative aspect-square w-full bg-white rounded-xl overflow-hidden shadow-sm border border-gray-200 group"
                    ...
                    <img :src="currentImage"
                        ...
                        :class="{ 'scale-150': isZoomed }" alt="Main product image" id="mainImage" />

                    <!-- Zoom Indicator -->
                    <div
                        class="absolute bottom-4 right-4 bg-black/80 text-white px-3 py-1.5 rounded-full text-sm flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                ...
                <div class="mt-6">
                    <div class="grid grid-cols-4 sm:grid-cols-5 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                        ...
                        <button @click="changeImage('{{ URL::to('storage/' . $record->main_image_url) }}')"
                            ...
                            :class="currentImage === '{{ URL::to('storage/' . $record->main_image_url) }}' ?
                                'border-gray-900 ring-2 ring-gray-200' : 'border-gray-200 hover:border-gray-400'">
                        ...
                        @foreach ($record->images as $image)
                            <button @click="changeImage('{{ URL::to('storage/' . $image->image_url) }}')"
                                class="relative aspect-square group cursor-pointer rounded-lg overflow-hidden border-2 transition-all duration-200"
                                :class="currentImage === '{{ URL::to('storage/' . $image->image_url) }}' ?
                                    'border-gray-900 ring-2 ring-gray-200' : 'border-gray-200 hover:border-gray-400'">
            ...
            <div class="bg-white rounded-xl p-6 lg:p-8 shadow-sm border border-gray-200">
                ...
                <div class="mb-6">
                    ...
                    <span
                        class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $record->status == 'in_stock' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                        {{ session('lang') == 'en'
                    ...
                    <div class="mb-8">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach ($record->categories as $category)
                                <span
                                    class="px-3 py-1 bg-gray-100 text-gray-800 rounded-full text-sm font-medium hover:bg-gray-200 transition-colors cursor-pointer">
        ...
        <div class="mt-12 bg-white rounded-xl p-6 lg:p-8 shadow-sm border border-gray-200" x-data="{ reviewsOpen: true }">
            <div class="flex items-center justify-between mb-6 cursor-pointer" @click="reviewsOpen = !reviewsOpen">
                ...
                <svg class="w-6 h-6 text-gray-400 transition-transform duration-200" :class="{ 'rotate-180': reviewsOpen }"
                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                </svg>
            </div>

            <div x-show="reviewsOpen" x-transition:enter="transition ease-out duration-300"
                ...
                <div class="space-y-6">
                            ...
                            <div class="flex items-center justify-between mb-2">
                                <div class="flex items-center gap-3">
                                    <div class="flex text-yellow-400">
                                        @for ($i = 1; $i <= 5; $i++)
                                            <span
                                                class="text-sm {{ $i <= $comment->rate ? 'text-yellow-400' : 'text-gray-300' }}">★</span>
                                        @endfor
                                    </div>
                                    <span
                                        class="text-sm font-medium text-gray-900">{{ $comment->customer ?? 'Anonymous' }}</span>
                                </div>
                    ...
                    @empty
                        <div class="text-center py-8 text-gray-500">
                            ...
                            <p>{{ session('lang') == 'en' ? 'No reviews yet. Be the first to review this product!' : 'لا توجد مراجعات بعد. كن أول من يراجع هذا المنتج!' }}
                            </p>
                        </div>
                    @endforelse
                </div>
