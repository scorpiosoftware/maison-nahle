<div id="default-carousel" class="relative w-full overflow-hidden  shadow-lg wowDiv" data-carousel="slide"
        ...
        @foreach ($carousel->images as $index => $image)
            ...
            <div class="hidden duration-700 ease-in-out absolute inset-0" data-carousel-item>
                ...
                <img src="{{ URL::to('storage/' . $image->url) }}"
                    class="absolute w-full h-full object-cover object-center" alt="Carousel image {{ $index + 1 }}"
                    loading="lazy">
            </div>
            <img src="{{ URL::to('storage/' . $image->url) }}" alt="">
        @endforeach
    </div>
